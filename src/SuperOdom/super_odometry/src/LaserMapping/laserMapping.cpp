...

{
        ...
        /*
            Added a this Below code from AI
            chatGPT START
        */
        
        // // --- Build a LOCAL odom (odom->base_footprint) by anchoring to the first map pose ---
        // const char* ODOM_FRAME = "odom";
        // const char* BASE_FOOTPRINT = "base_footprint";

        // // 1) Capture the first map pose as the odom origin
        // if (!has_lio_origin_) 
        // {
        //     q_map_odom0_ = q_w_curr;   // map->base at first frame (rotation)
        //     t_map_odom0_ = t_w_curr;   // map->base at first frame (translation)
        //     has_lio_origin_ = true;
        // }

        // // 2) Express current map pose in the odom frame:
        // //    T_odom_base = inv(T_map_odom0) * T_map_base
        // Eigen::Quaterniond q0_inv = q_map_odom0_.conjugate();
        // Eigen::Vector3d t_local   = q0_inv * (t_w_curr - t_map_odom0_);
        // Eigen::Quaterniond q_local = q0_inv * q_w_curr;
        // q_local.normalize();

        // // 3) Publish local odom
        // nav_msgs::msg::Odometry lio_odom;
        // lio_odom.header.stamp = rclcpp::Time(timeLaserOdometry*1e9);
        // lio_odom.header.frame_id = ODOM_FRAME;           // parent
        // lio_odom.child_frame_id  = BASE_FOOTPRINT;       // child

        // lio_odom.pose.pose.position.x = t_local.x();
        // lio_odom.pose.pose.position.y = t_local.y();
        // lio_odom.pose.pose.position.z = t_local.z();

        // lio_odom.pose.pose.orientation.x = q_local.x();
        // lio_odom.pose.pose.orientation.y = q_local.y();
        // lio_odom.pose.pose.orientation.z = q_local.z();
        // lio_odom.pose.pose.orientation.w = q_local.w();

        // // (Optional) reuse existing body-frame velocities you already computed
        // lio_odom.twist.twist.linear.x  = vel_b.x();
        // lio_odom.twist.twist.linear.y  = vel_b.y();
        // lio_odom.twist.twist.linear.z  = vel_b.z();
        // lio_odom.twist.twist.angular.x = ang_vel_b.x();
        // lio_odom.twist.twist.angular.y = ang_vel_b.y();
        // lio_odom.twist.twist.angular.z = ang_vel_b.z();

        // pubLIOPrediction->publish(lio_odom);
