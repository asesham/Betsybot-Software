...
namespace super_odometry {
    ...
    void imuPreintegration::reset_graph() {
        ...
        optimizer.update(newFactors, newValues);
    ...
    bool imuPreintegration::build_graph(gtsam::Pose3 lidarPose, double curLaserodomtimestamp) {
        ...
        newFactors.add(pose_factor);
        newFactors.add(imu_factor);
        newFactors.add(gtsam::BetweenFactor<gtsam::imuBias::ConstantBias>(
                B(key - 1), B(key), gtsam::imuBias::ConstantBias(),
                gtsam::noiseModel::Diagonal::Sigmas(
                        sqrt(imuIntegratorOpt_->deltaTij()) * noiseModelBetweenBias)));
        newValues.insert(X(key), propState_.pose());
    ...
    sensor_msgs::msg::Imu imuPreintegration::imuConverter(const sensor_msgs::msg::Imu &imu_in) {
        ...
        RCLCPP_INFO_STREAM(this->get_logger(), "imu_Init->pitch_offset_gravity " \
        << imu_Init->pitch_offset_gravity << "imu_Init->roll_offset_gravity " << imu_Init->roll_offset_gravity);
        ...
        RCLCPP_INFO_STREAM(this->get_logger(), "BEFORE imu_laser_R_Gravity X lin accel: x: " << acc.x() \
            << "y: " << acc.y() << "z: " << acc.z());
    
        acc=imu_laser_R_Gravity*acc;

        RCLCPP_INFO_STREAM(this->get_logger(), "AFTER imu_laser_R_Gravity X lin accel: x: " << acc.x() \
            << "y: " << acc.y() << "z: " << acc.z());
   ...
   void imuPreintegration::imuHandler(const sensor_msgs::msg::Imu::SharedPtr imu_raw) {
    ...
    //RCLCPP_INFO_STREAM(this->get_logger(), "BEFORE Converter lin accel: x: " << imu_raw->linear_acceleration.x \
            << "y: " << imu_raw->linear_acceleration.y << "z: " << imu_raw->linear_acceleration.z);
    //thisImu = imuConverter(*imu_raw);
    //RCLCPP_INFO_STREAM(this->get_logger(), "AFTER Converter lin accel: x: " << thisImu.linear_acceleration.x \
            << "y: " << thisImu.linear_acceleration.y << "z: " << thisImu.linear_acceleration.z);
    
    // RCLCPP_INFO_STREAM(this->get_logger(), "imu_raw lin accel: " << imu_raw->linear_acceleration.x \
            << "thisImu lin accel" << thisImu.linear_acceleration.x);
    thisImu = *imu_raw;
