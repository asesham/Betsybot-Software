...
namespace super_odometry {
    ...
    {      
        ...
        if (config_.sensor == SensorType::VELODYNE || config_.sensor == SensorType::OUSTER) {
            ...
            auto dummy=0;
            // subLivoxCloud = 
            // this->create_subscription<livox_ros_driver2::msg::CustomMsg>(LASER_TOPIC, 20, 
    ...
    {          
        ...
        this->declare_parameter<int>("feature_extraction_node.scan_line", 8);
    ...
    {        
        ...
        if (meas_end_time <= lidar_end_time) // make sure imu message arrives after lidar message
        {
            ...
            RCLCPP_WARN(this->get_logger(), "Before lidarBuf.measMap_.size() = %d", lidarBuf.measMap_.size());
            //lidarBuf.rclean(meas_end_time);

            RCLCPP_WARN(this->get_logger(), "After lidarBuf.measMap_.size() = %d", lidarBuf.measMap_.size());
            /*while (lidarBuf.measMap_.size() >= 1 && lidarBuf.measMap_.rbegin()->first >= meas_end_time) {
                lidarBuf.measMap_.erase(lidarBuf.measMap_.rbegin());
            }*/

            //if (lidarBuf.measMap_.size() >= 1) return true;
            return false;
        }
        ...
        if (meas_start_time >= lidar_start_time)          
        {
            ...
            lidarBuf.clean(meas_start_time);
    ...
    void featureExtraction::undistortionAndFeatureExtraction()      
    {
        ...
        if ((LASER_IMU_SYNC_SCCUESS == true or LASER_CAMERA_SYNC_SUCCESS == true) and lidarBuf.getSize() > 0)
        {
            ...
            // RCLCPP_INFO(this->get_logger(), "Debug line 468");
    ...
    void featureExtraction::imuInitialization(double timestamp) {
        ...
        if(lidarBuf.getFirstTime(lidar_first_time) && timestamp > lidar_first_time + LIDAR_MESSAGE_TIME + 0.05) 
    ...
    void featureExtraction::imu_Handler(const sensor_msgs::msg::Imu::SharedPtr msg_in) {
        ...
        int32_t curr_secs = curr_time.seconds();
        int64_t curr_nanosecs = curr_time.nanoseconds();
        
        int32_t pcloud_secs = msg_in->header.stamp.sec;
        int32_t pcloud_nanosecs = msg_in->header.stamp.nanosec;
        int64_t pcloud_init_time = pcloud_secs * 1000000000 + pcloud_nanosecs;
    ...
    {  
        ...
        uint64_t pcloud_secs = laserCloudMsg->header.stamp.sec;
        uint64_t pcloud_nanosecs = laserCloudMsg->header.stamp.nanosec;
        uint64_t pcloud_init_time = pcloud_secs * 1000000000 + pcloud_nanosecs;
        rclcpp::Time curr_time = this->get_clock()->now();
        
        int32_t curr_secs = curr_time.seconds();
        uint64_t curr_nanosecs = curr_time.nanoseconds();
        if (frameCount == 0)
        {
            stored_curr_time = curr_nanosecs;
            pcloud_stored_init_time = pcloud_init_time;
            time_diff = curr_nanosecs - pcloud_init_time;
        }

        uint64_t calculate_time = pcloud_init_time + time_diff;

        laserCloudMsg->header.stamp.sec = (int32_t)(calculate_time/1000000000);
        laserCloudMsg->header.stamp.nanosec = (int32_t)(calculate_time % 1000000000);
